/*-----------------------------------------------------------------------------
  Name:  GPU_ALL_NN_MED.cu
  Desc:  This file contains the QNN kd-tree GPU kernel
         for kd-trees stored in median layout

  by Shawn Brown (shawndb@cs.unc.edu)
-----------------------------------------------------------------------------*/

#ifndef _GPU_ALL_NN_MED_H_
#define _GPU_ALL_NN_MED_H_
...
(
	...
	GPUNode_2D_MED	* kdTree,	// IN: KD Tree (search & query Nodes)
...
)
{
	...
	queryPoints[tidx].x = kdTree[qidx].pos[0];
	queryPoints[tidx].y = kdTree[qidx].pos[1];
