...
#ifndef _KD_GPU_QNN_H_
#define _KD_GPU_QNN_H_
...
/*---------------------------------------------------------
  ...
  Note: We rewrote the query to use "registers"
        instead of "shared memory" for storing the
		stack, queryPoint, and current node.
		Ultimately, this didn't pan out as the resulting
		performance was 3 times slower than using
		"shared memory"
---------------------------------------------------------*/
...
)
{
	...
	//__shared__ float4 queryPoints[KD_THREADS_PER_BLOCK];							// query point
	//__shared__ GPUNode_2D_MED currNodes[KD_THREADS_PER_BLOCK];					// current node
	//__shared__ GPU_Search searchStack[KD_STACK_SIZE][KD_THREADS_PER_BLOCK];	// search stack

	// Per Thread Local Parameters (move from Shared Memory to registers)
	float4 queryPoint;
	GPUNode_2D_MED currNode;
	GPU_Search searchStack[KD_STACK_SIZE];
	...
	searchStack[stackTop].splitVal  = 3.0e+38F;
	stackTop++;
	...
	while (stackTop != 0)
	{
		...
		if (currInOut == 1)	// KD_OUT
		{
			...
			splitValue = searchStack[stackTop].splitVal;	// Split Value of Parent Node
		...
		splitValue = ((currAxis == 0) ? currNode.pos[0] : currNode.pos[1]);
		diff  = splitValue - queryValue;
		diff2 = diff*diff;
		...
		dx = currNode.pos[0] - queryPoint.x;
		dy = currNode.pos[1] - queryPoint.y;
		...
		if (queryValue <= splitValue)
		{
			...
			if (diff2 < best.Dist)
			{
				...
				if (0xFFFFFFFF != currNode.Right)	// cInvalid
				{
					...
					searchStack[stackTop].splitVal  = splitValue;
					stackTop++;
				}
			}
